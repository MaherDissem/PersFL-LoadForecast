...
class ModelWrapper:
    ...
    def build_model(self):
        encoder = EncoderRNN(
            ...
            hidden_size=self.args.s2s_hidden_size,
            num_grulstm_layers=self.args.s2s_num_grulstm_layers,
        ...
        decoder = DecoderRNN(
            ...
            hidden_size=self.args.s2s_hidden_size,
            num_grulstm_layers=self.args.s2s_num_grulstm_layers,
            fc_units=self.args.s2s_fc_units,
            output_size=self.args.nbr_var,
        ).to(self.device)

        model = Net_GRU(
            encoder, decoder, target_length=self.N_output, device=self.device
        ).to(self.device)
        return model
