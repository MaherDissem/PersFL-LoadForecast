...
class ALA:
    ...
    def adaptive_local_aggregation(
        ...
        rand_num = int(
            rand_ratio * len(self.train_data.dataset)
        )  # same indices for all batches, could be improved TODO
        subset_indices = random.sample(range(len(self.train_data.dataset)), rand_num)
